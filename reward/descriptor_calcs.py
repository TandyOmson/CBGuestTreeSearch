...
def calc_autocorr_4m(mol):
    autocorr = rdMolDescriptors.CalcAUTOCORR2D(mol)
    # the autocorrelation vector is 192 elements, split into 4 algorithms of 48 elements, with 6 weighting groups each with 8 bond distances
    # elems 144-192 are Geary autocorrelations, elems 144-152 are weighted by atomic masses, 147 is 4th elem of GATSm
    try:
        return autocorr[147]
    except:
        return 0.0

def calc_rdf_10p(mol):
...
def get_descs_uncharged(mol):
    MORSE = rdMolDescriptors.CalcMORSE(mol)

    autocorr_4m = calc_autocorr_4m(mol)
    rdf_10p = calc_rdf_10p(mol)
    mor22v = MORSE[85]
    n_oxygen = count_oxygen_atoms(mol)
    
    return autocorr_4m, rdf_10p, mor22v, n_oxygen
