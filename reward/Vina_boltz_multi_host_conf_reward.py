...
class CrestVinaCalc():
    ...
    def run_vina(self, repsdffile, ligsdffile, n_comps, inp):
        ...
        """
        receptors = Chem.SDMolSupplier(repsdffile, removeHs=False, sanitize=False)
        receptors = [add_nitrogen_charges(i) for i in receptors]
        ...
        complexens = []
        exos = []
        for count, i in enumerate(receptors):
            Chem.MolToMolFile(i, f"{self.rundir}/host_{count}.sdf")
            sp.run(["obabel", f"host_{count}.sdf", "-O", f"host_{count}.pdbqt", "-xrh"], cwd=self.rundir, stderr=open(f"{self.rundir}/vina.log", "w"))
        
            # Run Vina
            sp.run(["vina", f"--config={inp}", "--num_modes=100", "--energy_range=25", f"--receptor=host_{count}.pdbqt", f"--ligand=guest.pdbqt", f"--out=poses.pdbqt"], cwd=self.rundir, stdout=open(f"{self.rundir}/vina.log","a"))
            ...
            # use aligment docking as as last resort
                ...
                complexmols, vina_ens = self.MMFF94_vina_opt(align_pose, f"host_{count}.pdbqt")
                
        return complexmols, complexens
...
class Vina_reward(Reward):
    def get_objective_functions(conf):
        def VinaScore(mol):
            ...
            #conf["hostfile"] = f"{conf['vina_boltz_utils']}/cb7.sdf"
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                try:
                    ...
                    Mor29p, Mor24u, H046, Km = get_descs(docked_guest)
