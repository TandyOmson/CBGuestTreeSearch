""" Uses kde contours to cluster molceules, which are then used to evenly augment a datatset with randomized smiles
    Outputs the augmented dataset, and validation set indexes for the augmented dataset
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="",
        usage="python {os.path.basename(__file__)} -p PIPE_KDE_DIRECTORY -t TRAINING_SMIS -so AUGMENTED_SMILES_OUT -io AUGMENTED_VAL_SET_INDICES_OUT"
    )
    parser.add_argument(
        "-p",
        "--pipedir",
    ...
    # Get val set idxs for the existing dataset
    val_set_idxs = []
    empty_clusters = 0
    ...
    # Augment the dataset and add extra val set indices
    ...
    shift = 1.0 - min_weight
    class_weight_dict = {
        cls: weight + shift for cls, weight in class_weight_dict.items()
    }
    ...
    augmented_train_smis = copy(train_smis)
    augmented_val_set_idxs = copy(val_set_idxs)

    cluster_augmentation_dict = defaultdict(int)
    for cluster_num, weight in class_weight_dict.items():
        # sample (weight - 1)*cluster_size SMILES from each cluster and duplicate
        cluster_idxs = list(np.where(cluster_labels==cluster_num)[0])
        to_duplicate = random.choices(cluster_idxs, k=math.ceil((weight-1)*len(cluster_idxs)))
        ...
        augmented_val_set_idxs.extend([len(augmented_train_smis)-1 + i for i in range(1, int(val_split*len(to_duplicate)))])
    
    print(f"Augmented dataset from size {len(train_smis)} to {len(augmented_train_smis)}")
    print(f"Augmented validation set from size {len(val_set_idxs)} to {len(augmented_val_set_idxs)}")
    
    with open(args.smis_outfile, "w") as fw:
        for s in augmented_train_smis:
            fw.write(f"{s}\n")

    with open(args.idxs_outfile, "w") as fw:
        for i in augmented_val_set_idxs:
            fw.write(f"{i}\n")
    ...
    cluster_augmentation_lens = []
    for i in np.unique(cluster_labels):
        try:
            cluster_augmentation_lens.append(cluster_augmentation_dict[i])
        except:
            cluster_augmentation_lens.append(0)

    inset_ax = ax.inset_axes([0.1, 1.05, 0.8, 0.2])
    inset_ax.bar(np.unique(cluster_labels), cluster_lens, color="blue", alpha=0.7, label="clusters")
    inset_ax.bar(np.unique(cluster_labels), cluster_augmentation_lens, bottom=cluster_lens, color="orange", alpha=0.7, label="augmented")
    inset_ax.legend(bbox_to_anchor=(1.0, 0.5), loc="center left")
    inset_ax.set_ylabel("Cluster Size")
    inset_ax.set_ylim(0, max(np.array(cluster_augmentation_lens) + np.array(cluster_lens)) + 10)
