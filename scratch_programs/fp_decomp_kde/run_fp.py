...
# MCTS data
df_mcts = pd.read_csv(f"/home/andyt/DProjects/DMCTS/VINA_ChemTSv2/result/24_07_25/result_C0.7.csv")
...
#        plt.tight_layout()
    ...
    def save_decomp_coords(self, X_decomp, outfile):
        np.save(outfile, np.array(X_decomp))
        return
...
for count, (key, decomper) in enumerate(decompers.items()):
    ...
    # Save decomposed coordinates of training set
    decomper.save_decomp_coords(X_decomp_pccp, "X_{}.npy".format(key))
